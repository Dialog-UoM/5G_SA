...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action='store',
        required=True,
        choices=['start-basic', 'start-basic-vpp', 'stop-basic', 'stop-basic-vpp','start-gnb','start-ue','stop-gnb','stop-ue'],
        help='Functional Comopnent of 5G Network ("start-basic"|"start-basic-vpp"|"stop-basic"|"stop-basic-vpp"|"start-gnb"|"start-ue"|"stop-gnb"|"stop-ue")',
    )
...
if __name__ == '__main__':
    ...
    elif args.type == 'start-gnb':
        cmd = f'docker-compose -f docker-compose-gnbue.yaml up -d oai-gnb'
        res = run_cmd(cmd, False)
    elif args.type == 'start-ue':
        cmd = f'docker-compose -f docker-compose-gnbue.yaml up -d oai-nr-ue'
        res = run_cmd(cmd, False)
    elif args.type == 'stop-gnb':
        cmd = f'docker-compose -f docker-compose-gnbue.yaml down -d oai-gnb'
        res = run_cmd(cmd, False)
    elif args.type == 'stop-ue':
        cmd = f'docker-compose -f docker-compose-gnbue.yaml down -d oai-nr-ue'
        res = run_cmd(cmd, False)
